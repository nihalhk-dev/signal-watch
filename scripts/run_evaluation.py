...
Corre la evaluación completa sobre el banco sintético real y produce:
  · outputs/tables/curvas_retardo.csv  — tabla larga, una fila por
    (metrica, detector, punto de operacion, escenario, delta)
...
for tipo in ["auc", "psi"]:
    print(f"\n{'='*70}\n{tipo.upper()}\n{'='*70}")
    puntos = construir_curvas_para_metrica(
        tipo_metrica=tipo,
        ruta_calibracion_obs=PATHS.data_gold_synthetic / "calibracion.parquet",
        ruta_calibracion_gt=PATHS.data_gold_synthetic / "calibracion_ground_truth.parquet",
        ruta_evaluacion_obs=PATHS.data_gold_synthetic / "evaluacion.parquet",
        ruta_evaluacion_gt=PATHS.data_gold_synthetic / "evaluacion_ground_truth.parquet",
